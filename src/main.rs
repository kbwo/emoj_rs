...
fn main() {
    ...
    while let Ok(ev) = term.poll_event() {
        ...
        if let Ok(res) = search(&search_query.to_string().as_str()) {
            search_result = res;
...
pub fn select_emoji_by_key(
    emojis: Vec<String>,
    selected_index: usize,
) -> Result<String, Box<dyn std::error::Error>> {
    if emojis.len() > selected_index {
        Ok(emojis[selected_index].clone())
    } else if emojis.len() <= selected_index {
        Ok(emojis.clone().pop().unwrap())
    } else if !emojis.is_empty() {
        Ok(emojis[0].clone())
    } else {
        panic!("emojis or selected index was wrong")
    }
}
